...
Lista<TIPODATO>::Lista(int capacidad){
	/*cout << "Creando una lista vacía..." << endl;*/
	this->cuenta = 0;
	this->capacidad = capacidad;
	this->items = new TIPODATO();
...
void Lista<TIPODATO>::insertar(int indice, TIPODATO item){
	...
	for(int i = cuenta -1; i >= indice; i--){
		/*cout << "Desplazando el elemnto " << items[i] << " del indice ";
		cout << i << " al indice " << (i+1) << endl;*/
		this->items[i+1] = this->items[i];
	}
	
	/*cout << "Insertar elemento " << item << " en el indice " << indice << endl;*/
